...
int main(int argc, char *argv[])
{
  ...
  // Infotext
  printf(
      "INFO: Durch das Anlegen eines Unions wird Speicherplatz gespart, indem "
      "man\n"
      "      Datentypen 'ueberlappen' laesst dh. eine Art Speicherreservierung "
      "fuer\n"
      "      unterschiedliche Datentypen taetigt. Die Groesse ist dabei nur so "
      "maechtig wie\n"
      "      der groesste darin vorkommende Datentyp (In diesem Fall Int). \n"
      "      Es wird immer nur ein Datentyp gespeichert und wenn noetig, "
      "ueberschrieben\n\n"
      ...
      (int)sizeof(component.teststruct), (int)sizeof(component.txt),
  ...
  for (int i = 0; i < (int)sizeof(component.txt); i++)
  {
    printf(" Inhalt Union Byte[%d] (Hexadezimal): %x (Zeichen) %c\n", i,
           component.txt[i], component.txt[i]);
  }
  ...
  printf("\n-->Struktur beschrieben \n"
         "(Byte 1-3 wird nicht mit ueberschrieben, da Speicherbereich im "
         "Struct leer. (Siehe 4a)\n");
  printf("Inhalt teststruct.c: %c bzw. %x \n", component.teststruct.c,
         component.teststruct.c);
  printf("Inhalt teststruct.x: %d bzw. %x  \n", component.teststruct.x,
         component.teststruct.x);
