...
int main(int argc, char *argv[])
{
  ...
  while (1)
  {
    ...
    do
    {
      ...
      while (getchar() != '\n')
        ;
    } while (res != 1 || auftrag_msg.auftrag.funktno < 0 ||
             auftrag_msg.auftrag.funktno > 3);
    ...
    /* auf Ergebnis warten */
    if (msgrcv(msqid, &ergebnis_msg, sizeof(struct ergebnis),
               auftrag_msg.auftrag.erg_msgtyp, 0) < 0)
    {
      perror("msgrcv");
      exit(EXIT_FAILURE);
    }

    printf("Ergebnis: %s(%g) = %g \n", s[auftrag_msg.auftrag.funktno],
           auftrag_msg.auftrag.argument, ergebnis_msg.ergebnis.resultat);
  }
}
