...
{   /* liest Datei und schreibt in den Puffer */
    ...
    do {
        ...
        while (buffer.full == 1) {
            erg = pthread_cond_wait( &buffer.buffer_cond, 
			    &buffer.buffer_mutex);
            if (erg != 0) 
