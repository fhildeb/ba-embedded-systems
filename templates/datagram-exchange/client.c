...
int main(int argc, char *argv[])
{
  ...
  snprintf(client_addr.sun_path, sizeof(client_addr.sun_path) - 1, "%s%05d",
           CLIENT_SOCKET, getpid());
  addrlen =
      strlen(client_addr.sun_path) + offsetof(struct sockaddr_un, sun_path);
  ...
  addrlen =
      strlen(server_addr.sun_path) + offsetof(struct sockaddr_un, sun_path);
  ...
  while (1)
  {
    ...
    /* Auftrag senden */
    if ((nwritten =
             sendto(sockfd, &order, sizeof(struct order), 0,
    ...
    /* auf Ergebnis warten */
    if ((nread = recvfrom(sockfd, &result, sizeof(struct result), 0,
                          (struct sockaddr *)&server_addr2, &addrlen)) < 0)
    {
      perror("read");
      exit(EXIT_FAILURE);
    }
