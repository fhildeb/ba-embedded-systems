...
int main(void)
{
  ...
  while (1)
  {
    ...
    if ((result = recvfrom(s, line, sizeof(line), 0, (struct sockaddr *)&peerin,
                           &fromlen)) < 0)
    {
      if (errno == EINTR)
        continue;
    ...
    result = fprintf(stderr, "Datagram from %s Port %d\n",
                     inet_ntoa(peerin.sin_addr), ntohs(peerin.sin_port));
    assert(result > 0);

    if (strcmp(line, "instruct") == 0)
    { /* erste Anforderung ?? */
      result = snprintf(line, sizeof(line), "Hallo %s Port %d\n",
                        inet_ntoa(peerin.sin_addr), ntohs(peerin.sin_port));
      assert(result >= 0);

      result = sendto(s, line, strlen(line) + 1, 0, (struct sockaddr *)&peerin,
                      fromlen);
      assert(result >= 0);
    ...
    { /* alle anderen Anforderungen: Zeilen */
